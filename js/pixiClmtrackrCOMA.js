PIXI.loader.add('moc', "assets/Coma/COMA.moc3", { xhrType: PIXI.loaders.Resource.XHR_RESPONSE_TYPE.BUFFER })
PIXI.loader.add('texture', "assets/Coma/texture_00.png")
...
*/
		...
		if(pos){
			...
			params["ParamAngleX"] = 60 * (faceL - faceR) / (faceL + faceR);
			params["ParamAngleY"] = -60 * (vecL[0] * vecR[0] + vecL[1] * vecR[1]) / Math.sqrt(vecL[0] * vecL[0] + vecL[1] * vecL[1]) / Math.sqrt(vecR[0] * vecR[0] + vecR[1] * vecR[1]);
			params["ParamAngleZ"] = -60 * (pos[33][0] - pos[62][0]) / (pos[33][1] - pos[62][1]);
			params["ParamMouthOpenY"] = (pos[57][1] - pos[60][1]) / lipH - 0.5;
			//params["ParaM"] = 2 * (pos[50][0] - pos[44][0]) / (pos[30][0] - pos[25][0]) - 1;
			params["ParamEyeBallX"] = (pos[27][0] - pos[23][0]) / (pos[25][0] - pos[23][0]) - 0.5;
			params["ParamEyeBallY"] = (pos[27][1] - pos[24][1]) / eyeHL - 0.5;
			params["ParamEyeLOpen"] = 0.5 * eyeHL / lipH;
			params["ParamEyeROpen"] = 0.5 * eyeHR / lipH;
			params["ParamBrowLY"] = 1 * (pos[24][1] - pos[21][1]) / lipH - 4;
			params["ParamBrowRY"] = 1 * (pos[29][1] - pos[17][1]) / lipH - 4;
